...
resultsArea.on("click", function (event) {
  ...
  if (isResultElement) {
    ...
    displayArea.appendChild(bookHTML)
  }
});
...
function htmlToDisplay(book) {
  ...
  return `
    <h2>${book.title}</h2>
    <img src="http://books.google.com/books/content?id=${id}&printsec=frontcover&img=1&zoom=3&edge=curl&imgtk=AFLRE71s5GEuBSRbDToiNBDtAHH606N8B-vPdUp9DZPIkUEz5Zu8tWolDMb4yEihsi2PoRIhGrA9rEAnMlcpIaKonAPmRJ-wyPUqgQUAgnQhVwz8FCifKqh_S-KyZY_4VT2wprr9bXf5&source=gbs_api"></img>
    <h3><span class="description">${book.description}<span></h3>
  `;
 
}
// displayArea.append(htmlToDisplay(book))
